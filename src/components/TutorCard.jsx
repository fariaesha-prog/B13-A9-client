...
export default function TutorCard({ tutor }) {
  ...
  const subjectStyle = subjectColors[subject] || { bg: "bg-gray-100", text: "text-gray-600" };
  const modeStyle = modeColors[teachingMode] || { bg: "bg-gray-100", text: "text-gray-600" };
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 flex flex-col gap-3 hover:shadow-sm">
      <div className="flex items-center gap-3">
        {tutorPhoto ? (
          <img src={tutorPhoto} className="w-10 h-10 rounded-full object-cover" alt={tutorName} />
        ) : (
      ...
      <div className="text-xs text-gray-500">
        {Array.isArray(availableDays) ? availableDays.join(", ") : availableDays}
        {" · "}
        {startTime} – {endTime}
      </div>

      <div className="flex justify-between items-center">
        <span className="text-sm font-semibold text-gray-700">৳{hourlyFee}/hr</span>
